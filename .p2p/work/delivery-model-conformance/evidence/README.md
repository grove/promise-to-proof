...
- `live-host/`: the enclosing workflow's passed actual host check against the fixed reconstructed candidate, with real host-issued sessions and controller receipts. `development/live-host-inspection.json` records the independent development inspection of those raw records.
