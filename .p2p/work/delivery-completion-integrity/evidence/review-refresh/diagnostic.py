...
root=pathlib.Path('/Users/Shared/p2p-issue-24-6krj8of_/refresh25')
evidence=pathlib.Path('/private/tmp/p2p-review25-refresh-evidence')
base=json.loads((root/'records/base.json').read_text())
entries={e['path']:e for e in base}
raw=subprocess.check_output(['git','ls-tree','-rz','--full-tree','5a98bbcbeed9bbddce40d1b8158e0592bb10d13f'])
seen=set()
for row in raw.split(b'\0'):
 ...
 blob=subprocess.check_output(['git','cat-file','blob',obj.decode() if isinstance(obj,bytes) else obj])
