...
root=Path('/Users/Shared/p2p-issue-24-6krj8of_/refresh25'); meta=json.loads((root/'records/candidate.json').read_text())
...
for name,manifest in [('candidate',meta['manifest']),('base',json.loads((root/'records/base.json').read_text()))]:
 ...
 if name=='base':
  entries=subprocess.check_output(['git','ls-tree','-r',meta['comparison_base']],cwd='/Users/grove/projects/promise-to-proof').decode().splitlines()
  gitpaths=[]
  for line in entries:
   h,path=line.split('\t'); mode,typ,obj=h.split()
   if path.startswith('.p2p/'):continue
   gitpaths.append(path); e=next(x for x in actual if x['path']==path);assert mode==e['mode'];b=subprocess.check_output(['git','cat-file','blob',obj],cwd='/Users/grove/projects/promise-to-proof');assert b==(e['target'].encode() if typ=='blob' and mode=='120000' else base64.b64decode(e['content_base64']))
